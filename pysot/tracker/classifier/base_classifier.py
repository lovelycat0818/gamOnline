...
import math
import numpy as np
from pysot.tracker.classifier.optim import ConvProblem, FactorizedConvProblem
from pysot.tracker.classifier.libs import TensorList, operation
from pysot.tracker.classifier.libs import fourier, augmentation, dcf
from pysot.tracker.classifier.libs.plotting import show_tensor
from pysot.tracker.classifier.libs.optimization import GaussNewtonCG, ConjugateGradient, GradientDescentL2,  \
    ...
    def init_optimization(self, train_x, init_y):
        ...
        if self.update_projection_matrix:
            ...
            self.joint_problem = FactorizedConvProblem(
                self.init_training_samples, init_y, self.filter_reg, self.projection_reg,
